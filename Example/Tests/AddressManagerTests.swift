		...
		describe("AddressManagerTests") {
			...
			it("Manager should get address info") {
				...
				waitUntil(timeout: 10.0) { done in
					self.manager?.address(address: "Mxfc7281e9e1429c57d6cf02f193e9082e68dc052d",
																withSum: true,
			...
			it("Should show get info on multiple addresses") {
				...
				waitUntil(timeout: 10.0) { done in
					self.manager?.addresses(addresses: ["Mx184ac726059e43643e67290666f7b3195093f870", "Mxa82fdcc182090751d4b66e956810c7e1e2e92ecc"],
																	withSum: true,
																	completion: { (res, error) in
						expect(error).to(beNil())
						expect(res).toNot(beNil())
